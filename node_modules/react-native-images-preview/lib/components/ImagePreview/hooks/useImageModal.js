...
import { interpolate, interpolateColor, runOnJS, useAnimatedRef, useAnimatedStyle, useSharedValue, withTiming, } from 'react-native-reanimated';
import { StaticValues } from '../../../constants';
import { Colors } from '../../../theme';
const useImageModal = ({ modalConfig, setModalConfig, pinchZoomEnabled, doubleTapZoomEnabled, swipeDownCloseEnabled, }) => {
     ...
     * @param newScale
     */
    const updateTranslate = (newTranslateX, newTranslateY, newScale) => {
        'worklet';
    ...
    const doubleTapEvent = Gesture.Tap()
        .numberOfTaps(2)
        .enabled(doubleTapZoomEnabled !== null && doubleTapZoomEnabled !== void 0 ? doubleTapZoomEnabled : true)
        .onEnd(eventData => {
        if (scale.value !== 1) {
            resetValues();
        }
            ...
            saveScale.value = 2;
            translateX.value = withTiming(((WINDOW_WIDTH / 2 - eventData.x) * 1) / 2);
            translateY.value = withTiming(((WINDOW_HEIGHT / 2 - eventData.y) * 1) / 2);
    ...
    const pinchGestureEvent = Gesture.Pinch()
        .enabled(pinchZoomEnabled !== null && pinchZoomEnabled !== void 0 ? pinchZoomEnabled : true)
        .onChange(eventData => {
    ...
    const modalAnimatedStyle = useAnimatedStyle(() => ({
        backgroundColor: interpolateColor(colorOffset.value, [0, 1], [Colors.transparent, Colors.black]),
    }));
    /**
     * Use to animate size and position of image
     */
    const imageAnimatedStyle = useAnimatedStyle(() => ({
        height: interpolate(offset.value, [0, 1], [modalConfig.height, WINDOW_HEIGHT]),
